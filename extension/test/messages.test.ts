...
test("returns only safe domain options and generated alias", async () => {
  const d = deps();
  await expect(handleContentMessage({ type: "hme:domain-options" }, sender, d)).resolves.toEqual({ ok: true, domains: ["one.example"], defaultDomain: "one.example" });
  await expect(handleContentMessage({ type: "hme:generate", domain: "one.example" }, sender, d)).resolves.toEqual({ ok: true, alias: "shop@one.example" });
  expect(d.createRich).toHaveBeenCalledWith(expect.anything(), { domain: "one.example", format: "random_characters", description: "site.example" });
...
test("derives only the canonical top-level hostname for inline creation", async () => {
  ...
  await handleContentMessage({ type: "hme:generate", domain: "one.example" }, from, d);
  expect(d.createRich).toHaveBeenCalledWith(expect.anything(), { domain: "one.example", format: "random_characters", description: "accounts.example.com" });
});
...
test.each([undefined, "not a URL", "file:///private/path", "https:///", "https://bad_host.example/form"])("omits description for unusable top-level URL %s", async (url) => {
  ...
  await expect(handleContentMessage({ type: "hme:generate", domain: "one.example" }, from, d)).resolves.toEqual({ ok: true, alias: "shop@one.example" });
  expect(d.createRich).toHaveBeenCalledWith(expect.anything(), { domain: "one.example", format: "random_characters" });
});
...
test.each([
  ...
  const d = deps();
  await handleContentMessage({ type: "hme:generate", domain: "one.example" }, { ...sender, tab: { id: 3, url } } as chrome.runtime.MessageSender, d);
  expect(d.createRich).toHaveBeenCalledWith(expect.anything(), { domain: "one.example", format: "random_characters", description: hostname });
});
...
test("rejects an inline alias returned for a different domain without leaking URLs", async () => {
  ...
  const response = await handleContentMessage({ type: "hme:generate", domain: "one.example" }, { ...sender, tab: { id: 3, url: "https://private.example/path?secret=yes" } } as chrome.runtime.MessageSender, d);
