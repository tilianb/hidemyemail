...
If missing (no rows), add via the dashboard at `https://hidemyemail.tburg.workers.dev` (Domains tab → Add domain: `test.hidemyemail.dev`, destination: `16045812+tilianb@users.noreply.github.com`).

Or via D1 CLI:
```bash
cd worker && npx wrangler d1 execute hidemyemail \
  --command "INSERT INTO domains (domain, default_destination, active, created_at) VALUES ('test.hidemyemail.dev', '16045812+tilianb@users.noreply.github.com', 1, unixepoch('now') * 1000)" \
  --remote
```
...
Expected email in `16045812+tilianb@users.noreply.github.com` inbox:
