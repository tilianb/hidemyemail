...
// last_used_at is a display timestamp — refresh it at most once a minute so
// polling clients (browser extensions) don't turn every read into a D1 write.
 ...
 * tokens and for keys whose owner is disabled or tombstoned. Refreshes
 * last_used_at on success (throttled to one write per minute per key).
 */
export async function authenticateApiKey(db: D1Database, token: string): Promise<ApiKeyAuth | null> {
  if (!token.startsWith(TOKEN_PREFIX)) return null;
  const hash = await sha256Hex(token);
  const row = await db.prepare(
    "SELECT k.id, k.name, k.created_at, k.last_used_at, k.user_id, u.active, u.deleted_at " +
    "FROM api_keys k JOIN users u ON u.id = k.user_id WHERE k.token_hash = ?"
  ).bind(hash).first<{ id: number; name: string; created_at: number; last_used_at: number | null; user_id: number; active: number; deleted_at: number | null }>();
  if (!row || row.active === 0 || row.deleted_at != null) return null;
  const now = Date.now();
  if (row.last_used_at == null || now - row.last_used_at >= LAST_USED_WRITE_INTERVAL_MS) {
