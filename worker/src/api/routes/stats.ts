...
export function statsRoutes() {
  ...
  r.get("/stats", async (c) => {
    ...
    const userRow = await db.prepare("SELECT name FROM users WHERE id = ?").bind(userId).first<{ name: string | null }>();
    const userName = userRow?.name || (userId === 1 ? "Admin" : `User #${userId}`);
