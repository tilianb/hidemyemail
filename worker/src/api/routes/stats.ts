...
export function statsRoutes() {
  ...
  r.get("/stats", async (c) => {
    ...
    const userRow = await db.prepare("SELECT username, name FROM users WHERE id = ?").bind(userId).first<{ username: string | null; name: string | null }>();
    // Display precedence: self-set username → admin label name → fallback.
    const userName = userRow?.username || userRow?.name || (userId === 1 ? "Admin" : `User #${userId}`);
    return c.json({ totals: { aliases: aliases?.n ?? 0, active: active?.n ?? 0 }, last24h, topAliases: top.results ?? [], isAdmin: userId === 1, userName });
  });
  return r;
}
